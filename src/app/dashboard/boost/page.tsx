...
export default function BoostPage() {
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-10 pb-20">
            {/* Hero Section */}
            <div className="relative rounded-[2.5rem] overflow-hidden bg-surface border border-border p-8 lg:p-12">
                ...
                <div className="relative z-10 flex flex-col lg:flex-row items-center gap-12">
                    <div className="flex-1 space-y-6">
                        <Badge className="bg-primary/20 text-primary border-none px-4 py-1.5 rounded-full font-bold text-[10px] uppercase tracking-[0.2em]">Visibility Engine 2.0</Badge>
                        <h1 className="text-4xl lg:text-6xl font-bold tracking-tighter uppercase leading-[0.9]">
                            Get <span className="text-primary font-medium">Noticed</span> <br /> 
                        ...
                        <div className="flex flex-wrap gap-4 pt-4">
                            <div className="flex items-center gap-2 bg-background border border-border px-4 py-2.5 rounded-2xl">
                                <TrendingUp className="w-4 h-4 text-success" />
                                <span className="text-[10px] font-bold uppercase tracking-widest">3x Conversion</span>
                            </div>
                            <div className="flex items-center gap-2 bg-background border border-border px-4 py-2.5 rounded-2xl">
                    ...
                    <div className="w-full lg:w-72 aspect-[4/5] relative">
                         <div className={`absolute inset-0 rounded-[2.5rem] rotate-6 border blur-sm transition-all duration-700 ${hasActive ? 'bg-success/10 border-success/25' : 'bg-primary/20 border-primary/30 rotate-6 group-hover:rotate-3'}`} />
                         <div className="relative h-full bg-surface border border-border rounded-[2.5rem] p-6 flex flex-col justify-between overflow-hidden">
                             <div className="flex justify-between items-start">
                                 <div className={`p-2.5 rounded-2xl ${hasActive ? 'bg-success/10' : 'bg-background'}`}>
                                     <Zap className={`w-6 h-6 ${hasActive ? 'text-success fill-green-500/20' : 'text-primary fill-primary/20'}`} />
                                 </div>
                                 <Badge className={`${hasActive ? 'bg-success/10 text-success' : 'bg-gold/20 text-primaryDark'} border-none font-bold text-[8px] uppercase tracking-widest px-2 py-0.5`}>
                             ...
                             <div className="space-y-4">
                                 <div className="space-y-1">
                                     <p className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground/60">Algorithm Rank</p>
                                     <h3 className="text-2xl font-bold text-foreground">{hasActive ? '#1 Priority' : 'Top 40%'}</h3>
                                 </div>
                                 <div className="w-full bg-background h-1.5 rounded-full overflow-hidden">
                                     <motion.div 
                                        initial={{ width: 0 }}
                                        animate={{ width: hasActive ? "100%" : "30%" }}
                                        className={`h-full ${hasActive ? 'bg-success/10 shadow-[0_0_10px_rgba(34,197,94,0.5)]' : 'bg-primary shadow-[0_0_10px_rgba(224,30,90,0.5)]'}`} 
                                     />
                                 </div>
                                 <p className={`text-[8px] font-bold uppercase tracking-widest font-medium ${hasActive ? 'text-success' : 'text-primary'}`}>
            ...
            <div className="space-y-8">
                <div className="text-center space-y-2">
                    <h2 className="text-2xl lg:text-3xl font-bold tracking-tight uppercase">Select Your <span className="text-primary font-medium">Accelerator</span></h2>
            ...
            <div className="bg-primary/5 rounded-[2rem] p-10 flex flex-col items-center text-center space-y-6">
                ...
                <div className="max-w-xl space-y-2">
                    <p className="text-xs font-bold uppercase tracking-widest text-foreground font-medium">&quot;My profile views jumped from 20 to 500 in just 2 days. I found my match within a week of boosting!&quot;</p>
                    <p className="text-[9px] font-bold uppercase tracking-tighter text-muted-foreground/60">— Surbhi V., Raipur</p>
                </div>
                <div className="flex items-center gap-2">
                    <ShieldCheck className="w-4 h-4 text-primary" />
                    <span className="text-[8px] font-bold uppercase tracking-widest text-foreground/40">Secured with MatriShield™ Precision AI</span>
                </div>
            </div>
        </div>
    );

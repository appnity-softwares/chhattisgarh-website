...
import type { PhotoRequest, PhotoRequestStatus } from '@/types/api.types';
import {
...
export default function AdminPhotoRequestsPage() {
  ...
  const [statusFilter, setStatusFilter] = useState<PhotoRequestStatus | 'ALL'>('ALL' as PhotoRequestStatus | 'ALL');
  ...
  const fetchRequests = useCallback(async (page = 1) => {
    ...
    try {
      const data = await adminService.getPhotoRequests(page, 10, statusFilter === 'ALL' ? undefined : statusFilter, searchQuery);
      ...
      toast({ variant: 'destructive', title: 'Error', description: errorMsg.message || 'Failed to load photo requests' });
    } finally {
      setIsLoading(false);
    }
  ...
  const handleApprove = async (requestId: number) => {
    try {
      await adminService.updatePhotoRequest(requestId, 'APPROVED', 'Approved by admin');
      toast({ title: 'Success', description: 'Photo request approved' });
      fetchRequests();
    } catch (err: unknown) {
      const errorMsg = err as { message?: string };
  ...
  const handleBlockUser = async (userId: number) => {
    try {
      await adminService.banUser(userId.toString(), 'Blocked due to inappropriate photo requests');
    ...
    try {
      if (actionDialog.type === 'approve') {
        for (const requestId of selectedRequests) {
          await adminService.updatePhotoRequest(requestId, 'APPROVED', 'Bulk approved by admin');
        }
        toast({ title: 'Success', description: `${selectedRequests.size} requests approved` });
      } else if (actionDialog.type === 'reject') {
        for (const requestId of selectedRequests) {
          await adminService.updatePhotoRequest(requestId, 'REJECTED', 'Bulk rejected by admin');
        }
  ...
  return (
    <AdminPageWrapper
      ...
      description="Monitor and manage user photo view requests"
    >
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle>Photo Requests ({pagination.total})</CardTitle>
          </CardHeader>
          <CardContent>
            {isLoading ? (
              ...
              <div className="text-center py-12">
                ...
                <h3 className="text-lg font-semibold text-muted-foreground">No photo requests found</h3>
                <p className="text-muted-foreground mt-2">
                  {statusFilter === 'ALL' 
                    ? 'No photo requests match your current filters' 
                    : `No ${statusFilter.toLowerCase()} photo requests found`
                  }
                </p>
              </div>
            ) : (
                      ...
                      <TableRow key={request.id}>
                        ...
                        <TableCell>
                          <div className="flex items-center space-x-3">
                            ...
                            <div>
                              <div className="font-medium">{request.requester?.profile?.firstName} {request.requester?.profile?.lastName}</div>
                        ...
                        <TableCell>
                          <div className="flex items-center space-x-3">
                            ...
                            <div>
                              <div className="font-medium">{request.requestee?.profile?.firstName} {request.requestee?.profile?.lastName}</div>
                              <div className="text-sm text-muted-foreground">{request.requestee?.email}</div>
                            </div>
                          </div>
                        </TableCell>
                        <TableCell>
      ...
      <AlertDialog open={actionDialog.isOpen} onOpenChange={(open) => setActionDialog({ ...actionDialog, isOpen: open })}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {actionDialog.type === 'approve' ? 'Approve Photo Requests' : 'Reject Photo Requests'}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {actionDialog.type === 'approve' 
                ? `Are you sure you want to approve ${selectedRequests.size} photo request${selectedRequests.size > 1 ? 's' : ''}?`
                : `Are you sure you want to reject ${selectedRequests.size} photo request${selectedRequests.size > 1 ? 's' : ''}?`
              }
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>

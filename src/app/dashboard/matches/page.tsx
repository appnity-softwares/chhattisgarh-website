...
function MatchCard({ match, type, onAccept, onReject, onCancel, isAccepting, isRejecting, isCancelling }: {
    ...
    const statusBadge = (
        <Badge className={`font-bold text-[8px] uppercase tracking-widest border px-3 py-1 shadow-lg ${
            match.status === 'ACCEPTED' 
                ? 'bg-success/10 text-foreground border-success/25'
                : match.status === 'PENDING' 
                ? 'bg-gold/20 text-black border-gold/30' 
        ...
        <div className="flex gap-2 w-full">
            {type === 'received' && match.status === 'PENDING' && (
                <>
                    <Button
                        ...
                        size="sm"
                        variant="outline"
                        className="flex-1 h-9 rounded-xl bg-foreground/60 border-border text-error hover:bg-error/10 font-bold text-[9px] uppercase tracking-widest"
                    >
                    ...
                    <Button
                        ...
                        size="sm"
                        className="flex-1 h-9 rounded-xl bg-primary/90 text-foreground hover:bg-primary font-bold text-[9px] uppercase tracking-widest shadow-lg"
                    >
            ...
            {type === 'sent' && (
                <Button
                    ...
                    size="sm"
                    variant="outline"
                    className="w-full h-9 rounded-xl bg-background border-border text-error hover:bg-error/10 font-bold text-[9px] uppercase tracking-widest"
                >
            ...
            {type === 'accepted' && (
                <Link href={`/dashboard/chat?userId=${userId}`} className="w-full">
                    <Button
                        size="sm"
                        className="w-full h-9 rounded-xl bg-success/10 text-foreground hover:bg-success/10 font-bold text-[9px] uppercase tracking-widest shadow-lg"
                    >
    ...
    return (
        <MatchCardWrapper>
            <ProfileCard 
                ...
                relationship={{ 
                    status: match.status === 'PENDING' ? (type === 'received' ? 'received' : 'sent') : match.status.toLowerCase(),
                    matchId: match.id 
                }}
    ...
    if (displayMatches.length === 0) {
        return (
            <div className="py-32 flex flex-col items-center text-center space-y-6">
                <div className="w-24 h-24 bg-background rounded-full flex items-center justify-center border border-border">
                ...
                <div className="max-w-md space-y-2">
                    ...
                    <p className="text-muted-foreground font-medium font-medium">
        ...
        <div className="space-y-8 pb-20">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 px-1">
                <div className="space-y-2">
                    <h1 className="text-3xl md:text-4xl font-bold tracking-tight uppercase text-foreground">My <span className="text-primary font-medium">Matches</span></h1>
            ...
            <Tabs defaultValue="received" className="w-full">
                <div className="flex items-center justify-between border-b border-border pb-0 mb-8 overflow-x-auto no-scrollbar">
                    <TabsList className="bg-transparent border-none p-0 h-auto gap-8">
                        <TabsTrigger 
                            value="received" 
                            className="bg-transparent border-none p-0 pb-4 text-sm font-bold uppercase tracking-widest text-muted-foreground data-[state=active]:text-primary data-[state=active]:border-b-4 data-[state=active]:border-primary rounded-none transition-all gap-2 relative"
                        >
                            <Sparkles className="w-4 h-4" />
                            Received
                            {pendingCount > 0 && (
                                <span className="absolute -top-1 -right-3 bg-primary text-white text-[9px] font-bold h-5 w-5 rounded-full flex items-center justify-center animate-pulse">
                                    {pendingCount}
                                </span>
                            )}
                        </TabsTrigger>
                        <TabsTrigger 
                            value="sent" 
                            className="bg-transparent border-none p-0 pb-4 text-sm font-bold uppercase tracking-widest text-muted-foreground data-[state=active]:text-primary data-[state=active]:border-b-4 data-[state=active]:border-primary rounded-none transition-all gap-2"
                        >
